<template lang="html">
  <ifbp-page>
    ...
    <ifbp-initiate-flow v-if="!this.showBtnOfSpzt && this.showBtn" ref="initiate" :templatePk="templatePk" @callBack="processCallback" :passValue="passValue=true" @afterAction="processAfterCallback"></ifbp-initiate-flow>
    <!-- 主体区域 -->
    <ifbp-main-area type="detail" style="margin-top:8px;">
      <ifbp-panel-group :navbar="true" :base-nav-bar-top="baseNavBarTop" :base-scroll-top="baseScrollTop">
        <!-- 主表区域 -->
        <ifbp-panel id="pageExpenseAccount" :icons="mainDetailBtns" title="报销单" :main-title="true">
          <ifbp-template
                      ref="pageExpenseAccountDetail"
                      tpl-id="pageExpenseDetailNew"
          ...
          <ifbp-form-button-area v-if="formEdit">
             <el-button :type="item.displayname ==='保存'?'primary':''"  @click="triggerClick(item)" v-for="item in mainFormBtns">{{item.displayname}}</el-button>
        ...
        <ifbp-dynamic-panel
          ...
          :dynamic-pagination="dynamicPagination"
...
export default {
  ...
  methods: {
    edit(){
      ...
      // this.pageExpenseAccountIcons[0].show = false;
    },
    // 请求主表数据
    requestPageDetail() {
      // debugger;
    ...
    processCallback(action){
      //action为按钮的类型,从流程的组件中传过来的,同意=>agreeAble,驳回=>rejectAble,拒绝=>refuseAble,指派=>assignAble...
      //可以根据自己的需要，调用流程组件中的方法
      this.$refs.initiate.customEvents();
    },
    // 流程按钮确定后的事件(撤回=>withDraw,同意=>agreeAble,驳回=>rejectAble,拒绝=>refuseAble,指派=>assignAble...)
  ...
  created(){
    this.getPageModel(function(pageModel) {
      // debugger;
      // 获取详情页按钮
      // var mainDetailAllBtns;
      this.mainDetailBtns = pageModel.getTitleRightButtons("pageExpenseDetailNew");
      this.mainFormBtns = pageModel.getFormBottomButtons("pageExpenseDetailNew");
      // this.getClick(this.mainDetailBtns);
