...
export default {//导出组件course-card
  ...
  data() {//页面初始化
    return {
      // 查询模板传入参数
      searchCode: "SS",//平台查询模板定义值
      searchData: "",
      // 查询时提示展示
      searchState: "",
      // UI模板传入参数
      tableFunnode: "BU01",//平台UI模板定义值
      tableNexuskey: "KCTemp",//平台UI模板定义值
      // 模板列表
    ...
    pageTableClick(icon, scope) {
      ...
      if (icon === 'print'){
        ...
        window.open('/ifbp-print/static/html/iprint/businessPreview.html?serverUrl='
                    ...
                    + '/ifbp-print/rest/printTemplate/replaceTemplateByScenecode?busicode=BU01'
                    + '&scenecode=' + this.sceneCode + '&pk=' + scope.row.id);
      }
    },
