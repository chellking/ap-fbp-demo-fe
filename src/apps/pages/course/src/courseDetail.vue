<template>
  <ifbp-page>
   ...
   <!--
    <process
      ref="initiate"
      :showBtnOfSpzt="showBtnOfSpzt"
      :templatePk="templatePk"
      @callBack="processCallback"
      :passValue="passValue=true"
      @afterAction="processAfterCallback"
    ></process>-->
...
<initiate-flow ref="initiate" @callBack="beforeCallback" :passValue="passValue=true"></initiate-flow>
</div>-->
    <!-- 主体区域 -->
    <ifbp-main-area type="detail" style="margin-top:8px;">
      <ifbp-panel-group
        navbar="true"
        :base-nav-bar-top="baseNavBarTop"
        :base-scroll-top="baseScrollTop"
      >
        <!-- 主表编辑页签 -->
        <ifbp-panel id="courseInfo" title="课程信息" :icons="courseInfoIcons" :main-title="true">
          <!-- UI模板组件 -->
          <!-- funnode参数指定平台UI模板定义值 nexuskey参数指定平台UI模板定义值 -->
          <ifbp-template
            ref="formRef"
            tpl-id="formId"
            :editable="formEdit"
...
import initiateFlow from 'ifbp-business/initiateFlow/initiateFlow.vue';

export default {
  //导出组件process
  components: { 'initiateFlow' :initiateFlow },
  ...
  data() {
    ...
    return {
      ...
      /* ===========  子表 课表  =============== */
      courseFormFunnode: "BU01",
      courseFormNexuskey: "Course",
      courseFormData: {},
  ...
  methods: {
    ...
    processAfterCallback(action) {
      if(action !== "undefined" && action){
         this.$http({
        url: "/ifbp-demo-web/course/approved", //后台业务实现接口-流程提交
        headers: {
          "Content-Type": "application/json"
        },
        dataType: "application/json",
    ...
    // 获取场景编码
    getSceneCode() {
      this.sceneCode = window.location.href
        .split("?")[1]
        .split("&")[0]
        .split("=")[1];
